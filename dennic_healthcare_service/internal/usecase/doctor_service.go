...
func (u newsDoctorSService) GetAllDoctorServices(ctx context.Context, page, limit int64, search string) ([]*entity.DoctorServices, error) {
	ctx, cancel := context.WithTimeout(ctx, u.ctxTimeout)

	defer cancel()

	ctx, span := otlp.Start(ctx, serviceNameDoctorServiceUseCase, serviceNameDoctorServiceUseCaseRepoPrefix+"Get all")
	span.SetAttributes(attribute.Key("doctorService").String(search))

	defer span.End()

	return u.repo.GetAllDoctorServices(ctx, page, limit, search)
}
